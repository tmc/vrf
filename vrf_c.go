...
func (sk VrfPrivkey) cProve(message Hashable) (proof VrfProof, ok bool) {
...
func (proof VrfProof) cHash() (hash VrfOutput, ok bool) {
	ret := C.crypto_vrf_proof_to_hash((*C.uchar)(&hash[0]), (*C.uchar)(&proof[0]))
	return hash, ret == 0
}

func (pk VrfPubkey) cVerifyBytes(proof VrfProof, msg []byte) (bool, VrfOutput) {
	var out VrfOutput
	// &msg[0] will make Go panic if msg is zero length
	m := (*C.uchar)(C.NULL)
	if len(msg) != 0 {
...
func (pk VrfPubkey) cVerify(p VrfProof, message Hashable) (bool, VrfOutput) {
